...
import { createSessionSchema, getUserSessionsSchema } from '../utils/validators.js';

const router = Router();

router.post('/', authenticate, validate(createSessionSchema), sessionCtrl.createSession);
router.get('/', authenticate, validate(getUserSessionsSchema, 'query'), sessionCtrl.getSessions);
